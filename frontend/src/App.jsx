...
function App() {
  ...
  const fetchData = async () => {
    ...
    const { data, error: queryError } = await supabase.from('sales').select('*')
  ...
  const dealSizeData = useMemo(() => {
    ...
    return Object.entries(deals).map(([dealsize, count]) => ({ dealsize, count }))
  }, [salesData])
      ...
      <div className="dashboard-shell">
        <header className="dashboard-header">
          <div>
            ...
            <p className="dashboard-subtitle">Track revenue, orders, and sales mix in one place.</p>
          </div>
        ...
        {error && <p className="dashboard-status error">Error loading data: {error}</p>}
